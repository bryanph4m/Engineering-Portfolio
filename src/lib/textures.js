...
/**
 * The desk calendar's rest-state face: a small painted month grid with one
 * day circled in red, the same "highlight" idiom the paper documents use
 * (lib/docTextures.js circleHighlight). Static and painted once — the desk
 * calendar's actual availability is real data fetched only once it's picked
 * up (ui/CalendarBooking), so this face is decoration, not content, and never
 * needs to know today's real date.
 */
export function calendarFaceTexture() {
  if (cache.calendarFace) return cache.calendarFace
  const W = 384
  const H = 480
  const c = canvas(W, H)
  const ctx = c.getContext('2d')
  ...
  ctx.textAlign = 'center'
  ctx.fillText('BOOK A MEETING', W / 2, 56)
  ...
  const gridX = 36
  const gridY = 104
  const cell = (W - gridX * 2) / cols
  ctx.strokeStyle = 'rgba(51,41,29,0.55)'
  ctx.lineWidth = 1.5
  let circled = null
  for (let r = 0; r < rows; r++) {
    for (let col = 0; col < cols; col++) {
      ...
      // A stable, deliberately-picked weekday cell (not the first row, not a
      // weekend column) so the circle reads as "a bookable day", not decor.
      if (r === 2 && col === 3) circled = { x: x + cell / 2, y: y + cell / 2 }
    }
  }
  ...
  if (circled) {
    ...
    ctx.lineWidth = 3
    ctx.beginPath()
    ctx.ellipse(circled.x, circled.y, cell * 0.62, cell * 0.46, -0.08, 0, Math.PI * 2)
    ctx.stroke()
  }

  ctx.fillStyle = '#7a5a2f'
  ctx.font = '30px "Architects Daughter", cursive'
  ctx.fillText('pick a day, pick a time', W / 2, gridY + rows * cell + 46)

  const tex = new THREE.CanvasTexture(c)
  tex.colorSpace = THREE.SRGBColorSpace
  tex.anisotropy = Math.min(4, QUALITY.anisotropy)
  cache.calendarFace = tex
  return tex
}
